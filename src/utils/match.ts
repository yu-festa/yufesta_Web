import type { AgeBand, MatchApplicationRequest, MatchSummary, MatchTagOption } from '../api/match.ts'
...
export function toMatchApplicationRequest(application: InstatingApplication, options: MatchTagOption[], consents: boolean[]): MatchApplicationRequest {
  ...
  if (!tags.length || tags.length > 3 || tags.some(tag => !tag) || new Set(tags).size !== tags.length) throw new Error('관심 태그를 1개에서 3개까지 선택해 주세요.')
  ...
  return {
    ...
    termsVersion: MATCH_TERMS_VERSION, privacyVersion: MATCH_PRIVACY_VERSION, ageConfirmed: true,
  }
}
