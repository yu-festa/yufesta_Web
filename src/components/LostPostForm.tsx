...
export default function LostPostForm({ onRegistered }: { onRegistered: (post: LostItem) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError('')
    try {
      const post = await createLostItem({ kind, description, placeText: placeText.trim(), occurredAt: occurredAt ? `${occurredAt}:00` : null })
      onRegistered(post)
  ...
  return <form onSubmit={submit} className="mt-7 pb-4" aria-label="분실물 등록" aria-busy={saving}>
    ...
    <button type="submit" disabled={saving || !title.trim() || !body.trim() || !placeText.trim() || descriptionLength > MAX_LOST_DESCRIPTION} className="mt-10 min-h-12 w-full rounded-lg bg-[#1554ff] px-4 py-3 text-base font-bold text-white disabled:opacity-50">{saving ? '등록 중…' : '등록하기'}</button>
  </form>
}
