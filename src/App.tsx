...
type Page = 'notices' | 'instating-edit' | 'entry' | 'main' | 'admin' | 'timetable' | 'performance' | 'cheers' | 'map' | 'lost' | 'lost-write' | 'lost-detail' | 'login' | 'instating-apply' | 'profile' | 'instating-result'
...
function getPageFromHash(): Page {
  ...
  if (window.location.hash === '#notices' || window.location.hash.startsWith('#notices/')) return 'notices'
  if (window.location.hash === '#instating-edit') return 'instating-edit'
...
const App = () => {
  ...
  const [noticeId, setNoticeId] = useState<number | null>(() => window.location.hash.startsWith('#notices/') ? Number(window.location.hash.slice('#notices/'.length)) : null)
  ...
  const requiresAuthentication = requestedPage === 'profile' || requestedPage === 'instating-result' || requestedPage === 'instating-apply' || requestedPage === 'instating-edit' || requestedPage === 'admin'
  ...
  const adminEntry = !!adminRole && (authenticatedLogin || ['main', 'profile', 'instating-result', 'instating-apply', 'instating-edit'].includes(requestedPage) || (requestedPage === 'entry' && festivalOpened))
  ...
  useEffect(() => {
    const syncPage = () => {
      ...
      setNoticeId(window.location.hash.startsWith('#notices/') ? Number(window.location.hash.slice('#notices/'.length)) : null)
  ...
  function openNotice(id: number) {
    window.history.pushState(null, '', `/main#notices/${id}`)
    setNoticeId(id)
    setPage('notices')
  }
