      ...
      <div className={"text-[#222] [&_button]:cursor-pointer [&_button:disabled]:cursor-wait [&_button:disabled]:opacity-[0.55] [&_:is(button,_input,_textarea):focus-visible]:[outline:2px_solid_#1554ff] [&_:is(button,_input,_textarea):focus-visible]:outline-offset-[3px] [&_textarea.lost-found-field]:min-h-[160px] lost-found"}>
        ...
        {isWriting ? <LostPostForm onRegistered={registered} /> : <section className="pb-24" aria-labelledby="lost-intro">
          ...
          <nav className="mt-2.5 flex flex-wrap gap-3" aria-label="분실물 카테고리">
            {(['all', 'lost', 'found'] as const).map(value => <button key={value} type="button" className={"min-h-[32px] [border:1px_solid_#ddd] rounded-[7px] [padding:5px_15px] bg-white text-[12px] font-semibold [&[aria-pressed='true']]:[border-color:#1554ff] [&[aria-pressed='true']]:bg-[#1554ff] [&[aria-pressed='true']]:text-[white] lost-found-filter"} aria-pressed={filter === value} onClick={() => setFilter(value)}>{value === 'all' ? '전체' : lostPostLabels[value]}</button>)}
          </nav>
            ...
            {visiblePosts.map(post => <li key={post.id} className="border-b border-[#e5e5e5]">
              <button type="button" className="flex w-full items-center gap-4 py-4 text-left" onClick={() => setSelected(post)} aria-label={`${lostPostLabels[post.kind]} · ${post.title}`}>
                <div className="min-w-0 flex-1">
                  <span className={`inline-flex items-center rounded-[7px] [padding:4px_13px] text-[white] text-[11px] leading-[1.4] font-semibold [&.lost-found-badge--lost]:bg-[#7b9df9] [&.lost-found-badge--found]:bg-[#1554ff] lost-found-badge lost-found-badge--${post.kind}`}>{lostPostLabels[post.kind]}</span>
        ...
        <dialog ref={dialogRef} className="fixed inset-0 m-auto max-h-[85dvh] w-[min(440px,calc(100%-32px))] overflow-y-auto rounded-2xl border-0 bg-white p-6 text-[#222] shadow-xl backdrop:bg-[#11182766]" aria-labelledby="lost-detail-title" onClose={() => setSelected(null)} onClick={event => { if (event.target === event.currentTarget) dialogRef.current?.close() }}>
          {selected && <>
            <div className="flex items-center justify-between gap-2"><span className={`inline-flex items-center rounded-[7px] [padding:4px_13px] text-[white] text-[11px] leading-[1.4] font-semibold [&.lost-found-badge--lost]:bg-[#7b9df9] [&.lost-found-badge--found]:bg-[#1554ff] lost-found-badge lost-found-badge--${selected.kind}`}>{lostPostLabels[selected.kind]}</span><button type="button" className="grid size-10 shrink-0 place-items-center rounded-full" aria-label="게시글 닫기" onClick={() => dialogRef.current?.close()}><LostFoundIcon name="close" /></button></div>
