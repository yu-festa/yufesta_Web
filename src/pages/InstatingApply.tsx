...
export default function InstatingApply({ onHome, onProfile, onSubmitted, alreadyApplied = false, canApply = false, publishAt, initialApplication }: { onHome: () => void; onProfile: () => void; onSubmitted: () => Promise<void>; alreadyApplied?: boolean; canApply?: boolean; publishAt?: string; initialApplication?: MatchApplication }) {
  ...
  const [application, setApplication] = useState<Application>(() => initialApplication ? applicationToForm(initialApplication) : {
  ...
  async function next(event: FormEvent<HTMLFormElement>) {
    ...
    if (alreadyApplied && !editing) return
    if (!canApply) { setError('현재는 신청할 수 없어요. 접수 시간과 연결 상태를 확인해 주세요.'); return }
    if (step === 1 && (application.nickname.trim().length < 2 || application.nickname.trim().length > 8)) {
      setError('닉네임을 2~8자로 입력해 주세요.')
      return
    }
    ...
    if (step === 3) {
      ...
      setSaving(true)
      try {
        if (editing) await updateMyApplication(toMatchApplicationUpdate(application, interestOptions))
        else await createMatchApplication(toMatchApplicationRequest(application, interestOptions, consents))
      } catch (reason) {
  ...
  if (alreadyApplied && !editing && step !== 4) return (
    <AppLayout header={<InstatingHeader editing={editing} onHome={onHome} onProfile={onProfile} />}>
      ...
      <div className={"text-[#172039] [padding:24px_0_12px] [&_button:focus-visible]:[outline:2px_solid_#1554ff] [&_button:focus-visible]:outline-offset-[3px] [&_input:focus-visible]:[outline:2px_solid_#1554ff] [&_input:focus-visible]:outline-offset-[3px] [&_select:focus-visible]:[outline:2px_solid_#1554ff] [&_select:focus-visible]:outline-offset-[3px] [&_textarea:focus-visible]:[outline:2px_solid_#1554ff] [&_textarea:focus-visible]:outline-offset-[3px] [@media(prefers-reduced-motion:reduce)]:[&_*]:[transition:none] instating-apply"}>
        ...
        {step < 4 ? <form onSubmit={next}>
          <div className={"[margin:28px_0] [&_h2]:text-[25px] [&_h2]:font-[750] [&_h2]:tracking-[-1px] [&_h2]:leading-[1.4] [&_p]:text-[#7d8493] [&_p]:text-[13px] [&_p]:mt-[8px] [&_p]:leading-[1.6] [&_p]:break-keep instating-intro"}>
            ...
            {step === 1 && <span className={"block text-right text-[11px] text-[#1554ff] mt-[18px] instating-required-note"}>필수 항목 *</span>}
          </div>
          ...
          {step === 1 && <div className={"grid gap-[26px] instating-fields"}>
            <div className={"min-w-[0] [&_>_label]:block [&_>_label]:text-[15px] [&_>_label]:font-[650] [&_>_label]:mb-[10px] [&_legend]:block [&_legend]:text-[15px] [&_legend]:font-[650] [&_legend]:mb-[10px] [&_b]:text-[#1554ff] [&_>_input]:w-full [&_>_input]:[border:1px_solid_#e0e4ec] [&_>_input]:rounded-[8px] [&_>_input]:bg-white [&_select]:w-full [&_select]:[border:1px_solid_#e0e4ec] [&_select]:rounded-[8px] [&_select]:bg-white [&_>_input]:min-h-[50px] [&_>_input]:[padding:12px_14px] [&_>_input]:text-[16px] [&_select]:min-h-[50px] [&_select]:[padding:12px_14px] [&_select]:text-[16px] [&_input::placeholder]:text-[#a1a7b3] [&_textarea::placeholder]:text-[#a1a7b3] instating-field"}>
              ...
              <input id="instating-nickname" value={application.nickname} onChange={event => update('nickname', event.target.value)} placeholder="닉네임을 입력해주세요" required minLength={2} maxLength={8} autoComplete="nickname" />
            </div>
            <div className={"min-w-[0] [&_>_label]:block [&_>_label]:text-[15px] [&_>_label]:font-[650] [&_>_label]:mb-[10px] [&_legend]:block [&_legend]:text-[15px] [&_legend]:font-[650] [&_legend]:mb-[10px] [&_b]:text-[#1554ff] [&_>_input]:w-full [&_>_input]:[border:1px_solid_#e0e4ec] [&_>_input]:rounded-[8px] [&_>_input]:bg-white [&_select]:w-full [&_select]:[border:1px_solid_#e0e4ec] [&_select]:rounded-[8px] [&_select]:bg-white [&_>_input]:min-h-[50px] [&_>_input]:[padding:12px_14px] [&_>_input]:text-[16px] [&_select]:min-h-[50px] [&_select]:[padding:12px_14px] [&_select]:text-[16px] [&_input::placeholder]:text-[#a1a7b3] [&_textarea::placeholder]:text-[#a1a7b3] instating-field"}>
            ...
            <fieldset className={"min-w-[0] [&_>_label]:block [&_>_label]:text-[15px] [&_>_label]:font-[650] [&_>_label]:mb-[10px] [&_legend]:block [&_legend]:text-[15px] [&_legend]:font-[650] [&_legend]:mb-[10px] [&_b]:text-[#1554ff] [&_>_input]:w-full [&_>_input]:[border:1px_solid_#e0e4ec] [&_>_input]:rounded-[8px] [&_>_input]:bg-white [&_select]:w-full [&_select]:[border:1px_solid_#e0e4ec] [&_select]:rounded-[8px] [&_select]:bg-white [&_>_input]:min-h-[50px] [&_>_input]:[padding:12px_14px] [&_>_input]:text-[16px] [&_select]:min-h-[50px] [&_select]:[padding:12px_14px] [&_select]:text-[16px] [&_input::placeholder]:text-[#a1a7b3] [&_textarea::placeholder]:text-[#a1a7b3] instating-field"}><legend>나이대 (선택)</legend>
              <div className={"grid grid-cols-[1fr_1fr] gap-[10px] instating-options"}>{ages.map(age => <label className={"relative block cursor-pointer [&_input]:absolute [&_input]:w-[1px] [&_input]:h-[1px] [&_input]:opacity-[0] [&_>_span]:flex [&_>_span]:justify-center [&_>_span]:items-center [&_>_span]:gap-[8px] [&_>_span]:min-h-[46px] [&_>_span]:[border:1px_solid_#e0e4ec] [&_>_span]:rounded-[8px] [&_>_span]:text-[14px] [&_>_span]:[transition:background_.15s] [&_input[type=radio]_+_span::before]:[content:''] [&_input[type=radio]_+_span::before]:w-[15px] [&_input[type=radio]_+_span::before]:h-[15px] [&_input[type=radio]_+_span::before]:[border:1px_solid_#c3cad7] [&_input[type=radio]_+_span::before]:rounded-full [&_input:checked_+_span]:[border-color:#1554ff] [&_input:checked_+_span]:bg-[#f1f5ff] [&_input:checked_+_span]:text-[#1554ff] [&_input:checked_+_span]:font-[650] [&_input[type=radio]:checked_+_span::before]:[border:4px_solid_#1554ff] [&_input[type=radio]:checked_+_span::before]:bg-white [&_input:disabled_+_span]:opacity-[.4] [&_input:disabled_+_span]:cursor-not-allowed [&_input:focus-visible_+_span]:[outline:2px_solid_#1554ff] [&_input:focus-visible_+_span]:outline-offset-[3px] instating-option"} key={age}><input type="radio" name="age" value={age} checked={application.age === age} onChange={() => update('age', age)} /><span>{age || '선택 안 함'}</span></label>)}</div>
          ...
          <button className={"block w-full min-h-[52px] p-[14px] rounded-[8px] text-[15px] font-bold cursor-pointer mt-[28px] bg-[#1554ff] text-[white] [&:hover]:bg-[#1046db] [&:active]:[transform:scale(.99)] instating-primary"} type="submit" disabled={saving || !canApply || (application.tags.length > 0 && interestOptions.length === 0)}>{saving ? '저장 중…' : step === 3 ? editing ? '수정 완료하기' : '신청 완료하기' : '다음 단계로'}</button>
          ...
          <h2 ref={headingRef} tabIndex={-1}>{editing ? '신청 정보가 수정되었습니다!' : '신청이 완료되었습니다!'}</h2>
