...
import { cancelMyApplication, getMyMatchResults } from '../api/match'
import type { MatchSummary, MatchApplication } from '../api/match'

export default function Profile({ user, isPreview, onBack, onHome, onResult, onApply, onLogout, matchSummary, alreadyApplied = false, canApply = false, onChanged, application, onEdit }: { user: ProfileUser; isPreview: boolean; onBack: () => void; onHome: () => void; onResult: (id: string) => void; onApply: () => void; onLogout?: () => Promise<void>; matchSummary?: MatchSummary | null; alreadyApplied?: boolean; canApply?: boolean; onChanged: () => Promise<void>; application?: MatchApplication | null; onEdit: () => void }) {
  ...
  const loadResult = useCallback(async () => lastRoundSeq ? toMatchResult(await getMyMatchResults(lastRoundSeq)) : null, [lastRoundSeq])
  const latestResult = usePublicResource(loadResult)
    ...
    <AppLayout header={<div className="flex h-22 items-center"><HomeLogo onHome={onHome} /></div>}>
      <div className={"text-[#192135] pb-[12px] [&_button:focus-visible]:[outline:2px_solid_#1554ff] [&_button:focus-visible]:outline-offset-[3px] [&_summary:focus-visible]:[outline:2px_solid_#1554ff] [&_summary:focus-visible]:outline-offset-[3px] profile-page"}>
        ...
        <section className={"mt-[36px] profile-history"} aria-labelledby="profile-history-title">
          ...
          <p className={"text-[12px] text-[#8992a4] mt-[7px] break-keep profile-history-description"}>{isPreview ? '축제를 함께할 친구와의 설렘을 확인해보세요.' : '현재 회차 신청과 최근 발표 결과를 확인해보세요.'}</p>
        ...
        {!isPreview && <div className="mt-6 space-y-3">
          {alreadyApplied ? <><p className="text-center text-xs text-[#63708a]">이번 회차 신청 완료 · 마감 전까지 수정·취소할 수 있어요.</p>{application?.wantedSlot && <p className="text-center text-xs text-[#3159a3]">선호 공연: {application.wantedSlot.title} · {application.wantedSlot.stageName}</p>}{application?.needsSlotReselect && <p className="rounded-lg bg-[#edf3ff] p-3 text-center text-xs text-[#1554ff]">이전 공연을 현재 회차에서 사용할 수 없어요. 수정 화면에서 다시 선택해 주세요.</p>}<button type="button" disabled={!canApply || busy || !application} className="w-full rounded-xl border border-[#dce5f5] p-3 text-sm text-[#1554ff] disabled:opacity-40" onClick={onEdit}>신청 정보 수정하기</button><button type="button" disabled={!canApply || busy} className="w-full rounded-xl border border-red-200 p-3 text-sm text-red-700 disabled:opacity-40" onClick={() => { setAction('cancel'); setActionError(''); setNotice('') }}>신청 취소하기</button></>
          ...
          {lastRoundSeq && !alreadyApplied && <><ResourceStatus loading={latestResult.loading} error={latestResult.error} retry={latestResult.refresh} />{latestResult.data && !latestResult.loading && !latestResult.error && <MatchRejoin key={lastRoundSeq} data={latestResult.data} disabled={!canApply} onChanged={async () => { await onChanged(); await latestResult.refresh() }} />}</>}
          {!canApply && <p className="text-center text-xs text-[#63708a]">접수 기간이 아니거나 신청 상태를 확인 중이에요.</p>}
          {action && <div className="rounded-xl border border-[#dce5f5] p-4" role="group" aria-label="신청 취소 확인"><p className="text-sm">이번 회차 신청을 취소할까요?</p><div className="mt-3 flex gap-3"><button type="button" disabled={busy || !canApply} className="rounded-lg bg-[#1554ff] px-4 py-2 text-sm text-white disabled:opacity-40" onClick={() => void confirmAction()}>{busy ? '처리 중…' : '확인'}</button><button type="button" disabled={busy} className="rounded-lg border px-4 py-2 text-sm" onClick={() => setAction(null)}>돌아가기</button></div></div>}
