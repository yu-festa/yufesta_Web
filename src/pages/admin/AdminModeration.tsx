...
import AdminLostComments from '../../components/admin/AdminLostComments'
import type { CommentSelection } from '../../components/admin/AdminLostComments'

const targetLabels: Record<ContentReportTarget, string> = { CHEER: '응원', LOST_ITEM: '분실물', LOST_ITEM_COMMENT: '분실물 댓글' }
...
export default function AdminModeration() {
  ...
  const commentSection = useRef<HTMLDivElement>(null)
  const openComments = (selection: CommentSelection) => { setCommentSelection(selection); requestAnimationFrame(() => commentSection.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })) }
  ...
  const visibility = (type: ContentReportTarget, id: number, hidden: boolean) => {
    if (type === 'LOST_ITEM_COMMENT') { openComments({ commentId: id, hidden }); return }
    setConfirmation({
