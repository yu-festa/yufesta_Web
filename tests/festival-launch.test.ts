...
test('축제 오픈은 2026년 10월 1일 0시 한국 시간이다', () => {
  assert.equal(FESTIVAL_START_AT, '2026-10-01T00:00:00+09:00')
  assert.equal(new Date(FESTIVAL_START_TIMESTAMP).toISOString(), '2026-09-30T15:00:00.000Z')
})
...
test('개발용 시각은 명시적 시간대가 있어야 하며 잘못된 값은 공식 시각을 쓴다', () => {
  ...
  for (const value of [undefined, '', 'wrong', '2026-10-01T00:00:00', 'invalid+09:00']) assert.equal(resolveFestivalStart(value), FESTIVAL_START_TIMESTAMP)
})
