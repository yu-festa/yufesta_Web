...
- 요청한 테스트 발송 시각: **2026-09-26 18:10:00 한국 시간** (`2026-09-26T09:10:00Z`).
- 실제 축제 랜딩 종료 시각(10월 2일 0시)은 바꾸지 않습니다. 테스트 알림에는 ‘테스트’ 문구를 표시하고 클릭하면 `/main`으로 이동합니다.
- 비로그인 사용자가 랜딩의 ‘오픈 알림 받기’를 누르고 알림 권한을 허용하면 해당 브라우저의 푸시 구독을 QStash에 예약합니다. QStash의 저장 응답을 받은 뒤에만 신청 완료를 표시합니다.
...
| `PUSH_OPEN_AT` | `2026-09-26T18:10:00+09:00` |
| `PUSH_OPEN_MODE` | `test` |
...
3. **알림 신청 완료**와 **9월 26일 오후 6:10** 안내를 확인합니다. 버튼만 누르거나 알림 권한만 허용한 상태는 예약 완료가 아닙니다.
4. 사이트를 닫고 기기의 인터넷·알림 허용을 유지합니다.
5. 18:10 이후 테스트 알림을 확인하고 누르면 `/main`이 열리는지 확인합니다.
...
- `PUSH_OPEN_AT=2026-10-02T00:00:00+09:00`
...
- `npm run test:push`: 기존 테스트 푸시 13개와 예약·서명·암호화·시각·오류 처리 10개, 총 23개.
- 프로덕션 빌드 및 린트 검사.
- 실제 QStash 계정 연결·Production 배포·실기기 권한 허용 및 수신 확인은 설정 후 진행해야 합니다.
